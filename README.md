...
## 版本更新说明
  v4.2.0 版本增加了对转发请求的日志记录功能，并提供转发请求成功次数、失败次数、成功率和平均响应时间的统计。 
如需对转发请求进行个性化业务处理，可以对转发请求日志数据库表进行分析处理。
## 快速开始
1. 部署架构
...
#### 1.7. 账户锁定
##### 1.7.1. 原理介绍
用户登录系统时，输入密码错误多次后锁定账户，避免暴力破解密码，默认输入错误3次，咱未提供配置次数功能。
系统根据登录用户，在Redis中记录登录失败次数，登录失败累加计数，当失败次数超过3次后，触发锁定账户机制，
修改数据库锁定字段，并清空Redis锁定计数。
##### 1.7.2. 使用方式
软件自带功能，暂不支持配置功能，后续增加配置。
#### 1.8. 密码加密
##### 1.8.1. 原理介绍
